...
Elas não podem ser paradas, não podem ser mortas. Todos têm uma.

Se uma forma física é destruída, elas simplesmente renascem na próxima noite onde o luar serenamente banha a Terra.

  

A civilização evoluiu de acordo.

As cidades encontraram maneiras de se protegerem da luz do luar. Algumas cidades bloquearam o céu completamente, enquanto outros utilizaram a magia dos poderosos feiticeiros para criar barreiras que bloqueiam somente a luz da lua durante a noite.

A humanidade estava presa.

Até o momento, em que um sonho das estrelas brilhou cintilante, e levou a humanidade ao futuro no qual ela não poderia alcançar em sua solidão.

  

Foram criados os Anjos.

  

Autômatos mágicos, feitos de corpo, mente e coração.

O corpo é qualquer receptáculo capaz de segurar um ser.

A mente é uma runa, cravada em um presente das estrelas em uma linguagem que existe somente a elas.

O coração é um objeto banhado de sentimento e alma. É frequentemente uma obra de arte.

  

Com sua conexão celestial, os Anjos tinham o poder de mudar o mundo.

Através do polimorfismo, eles fisicamente transformam um objeto, para a forma de um objeto conectado no mundo eterno. Esta conexão tende a ser uma de utilidade, embora Anjos poderosos tenham a habilidade de ir além disso.

Por exemplo, um Anjo utiliza o polimorfismo em uma faca.

Uma faca é utilizada para preparar refeições. Logo, isto a conecta a um garfo. Entretanto, uma faca que é utilizada para ferir também está conectada a uma espada. Ambos são possíveis formas que um objeto pode ter através da transformação do polimorfismo.
...
E assim iniciou a era que permitiu que os humanos vivessem através das sombrias noites.

Anjos, os nossos protetores. Guardando na escuridão, como as próprias estrelas, guiando nosso tempo e espaço.